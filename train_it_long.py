...
def train(
    ...
    metric_logger.add_meter("lr", SmoothedValue(window=100, fmt="{value:.7f}"))
    loss_names = ["loss"]
    ...
    # log eval_freq
    logger.info(f'eval_freq: {eval_freq}')
    ...
    for i, (media_type, (image, text, instruction, _, frames_times)) in enumerate(iterator):
        ...
        with torch.cuda.amp.autocast(enabled=config.fp16):
            loss_dict = model(image, text, instruction, frames_times)
        ...
        if is_main_process() and (eval_freq > 0 and global_step % eval_freq == 0):
            model.eval()
            with torch.no_grad():
                avgs = []
                for dataset in ['Action Sequence', 'Object Interaction', 'State Change']:
                    avg = evaluate(model.module, 
                            [dataset],
                            base_frame_num=config.base_frame_num,
                            num_frm=-1, 
                            resolution=224,
                            target_video_length=-1,
                            max_num_frm=config.num_frames,
                            output_file=f'{config.output_dir}/eval_{global_step:06d}.json',
                            device=device,
                            )
                    avgs.append(avg)
                logger.info(f"Global step: {global_step} Evaluation: {np.mean(avgs)}, AS, OI, SC: {avgs}")
                # log into metric_logger
                metric_logger.update(eval=avg)
            
            if global_step > 0:

                logger.info(f"Epoch {epoch}")
                param_grad_dic = {
                    k: v.requires_grad for (k, v) in model.module.named_parameters()
                }
                ...
                torch.save(save_obj, join(config.output_dir, f"ckpt_step_{global_step:05d}.pth"))
...
def main(config):
    ...
    # # debug:
    # linear = torch.nn.Linear([16,10]).cuda()
    # x1 = torch.ones([1, 16]).cuda()
    # x2 = torch.ones([1, 16]).cuda()

    # with torch.no_grad():
    #     linear(x1)
    # print(linear(x2).grad_fn)
