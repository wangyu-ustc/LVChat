...
def read_frames_decord(
        video_path, num_frames, sample='rand', fix_start=None, 
        max_num_frames=-1, client=None, clip=None, base_num_frames=None, frames_times=1,
    ):
    if video_path.startswith('s3') or video_path.startswith('p2'):
        video_bytes = client.get(video_path)
    ...
    # Constrain the number of frames to make sure there is not too many frames sampled if the video is not long enough
    # num_frames = min(int(((duration // base_num_frames) + 1) * base_num_frames), num_frames)
    
    # np.ceil(duration / base_num_frames) * base_num_frames
    num_frames = min(int(np.ceil(duration / base_num_frames) * base_num_frames), num_frames) * frames_times
