        ...
        for _ in range(self.num_tries):
            try:
                ...
                if self.random_frames:
                    times = np.random.randint(1, self.num_frames // self.base_num_frames)
                    num_frames = times * self.base_num_frames
                else:
                    num_frames = self.num_frames
                
                probabilities = self.multiple_frames
                frames_times = np.random.choice(np.arange(len(self.multiple_frames)), p=probabilities) + 1

                frames, frame_indices, fps = self.video_reader(
                    data_path, num_frames, self.sample_type, 
                    max_num_frames=max_num_frames, client=self.client, clip=clip, base_num_frames=self.base_num_frames,
            ...
            if return_fps:
                ...
                return frames, index, sec, frames_times
            else:
                return frames, index, frames_times
        else:
            raise RuntimeError(
