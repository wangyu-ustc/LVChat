...
class ITImgTrainDataset(ImageVideoBaseDataset):
    ...
    def __init__(
        ...
        if 'videochatgpt' in ann_file[0]:

            all_files = set(os.listdir(self.data_root))

            self.anno = [
                anno for anno in self.anno if anno['video'].split("/")[-1] in all_files
            ]
        ...
        print("Dataset:", ann_file[0], self.num_examples)
...
class ITVidTrainDataset(ITImgTrainDataset):
    ...
    def __init__(
        self, ann_file, transform,
        num_frames=4, base_num_frames=4, random_frames=False,
    ...
    def __getitem__(self, index):
        try:
            ...
            video, index, sec, frames_times = self.load_and_transform_media_data_video(index, ann["image"], return_fps=True, clip=clip)
            ...
            return video, conversation, instruction, index, frames_times
        except Exception as e:
